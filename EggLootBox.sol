...
contract EggLootBox is LootBoxRandomness {
    ...
    function setEggLootBoxState(uint256 newLootBoxOptions, uint256 newEggOptions, uint256 newEggClasses) public {
        LOOTBOX_OPTIONS = newLootBoxOptions;
        _setLootBoxState(newEggOptions, newEggClasses);
    }
    
    function getEggLootBoxState() public view returns(uint256, uint256, uint256) {
        uint256 eggOptions;
        uint256 eggClasses;
        (eggOptions, eggClasses) = _getLootBoxState();
        return (LOOTBOX_OPTIONS, eggOptions, eggClasses);
    }
